...
// PA4: Safe Accessor that will return a black pixel (clampToEdge = false) or the
...
float FloatImage::smartAccessor(int x, int y, int z, bool clampToEdge) const
{
    if (clampToEdge){
        ...
        int y_temp;
        //cout<< "z = " <<z<<endl;
        x_temp = std::min(std::max(x,0),this->width() - 1);     // 减1 
        y_temp = std::min(std::max(y,0),this->height() - 1);
        return operator()(x_temp,y_temp,z);
    }
    else 
        return x >= 0 && x < this->width() && y >= 0 && y < this->height() && z >= 0 && z < this->channels() ? operator()(x,y,z) : 0.0f;
}
    ...
    for (unsigned int x = 0; x < width_; x++)
        for (unsigned int y = 0; y < height_; y++)
            for (unsigned int c = 0; c < outputchannels_; c++)
                operator()(x,y,c) = uint8_to_float(uint8_image[c + x*channels_ + y*channels_*width_]);
